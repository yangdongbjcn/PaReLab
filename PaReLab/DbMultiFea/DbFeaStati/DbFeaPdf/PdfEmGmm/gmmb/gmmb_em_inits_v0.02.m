function [estimate, varargout] = gmmb_em_inits(data, inits, varargin)

	comments();

	[conf, varargout] = initConf(varargin);
	...
	[N, D, C] = computeNdc(data, conf);
	
	warnFreeParas(data, N, D, C);
	
	[mu, sigma, weight, log_initialmix] = initGm(inits, data, conf, C);
	...
	[pcompx] = init(mu, sigma, weight, C, data);

while 1
	% one EM cycle
	
	[mu, sigma, weight, fixerloops] = maximum(pcompx, data, N, D, C);
	
	[pcompx, loglike] = expect(mu, sigma, weight, C, data);
	
	[log_covfixer2, log_loglikes, fixing_cycles] = judgeLoglike(conf, loglike, old_loglike, loops, ...
			log_covfixer2, log_loglikes, fixerloops, ...
			weight, mu, sigma, fixing_cycles);
...
function [conf, varargout_cell] = initConf(varargin_cell)
	% varargin_cell 
	...
	conf = getargs(conf, varargin_cell);
...
function [N, D, C] = computeNdc(data, conf)
	% data, conf
...
function warnFreeParas(data, N, D, C)
	% data, N, D, C
...
function [mu, sigma, weight, log_initialmix] = initGm(inits, data, conf, C)
	% inits, data, conf, C
...
function [pcompx] = init(mu, sigma, weight, C, data)
	% mu, sigma, weight, C, data
...
function [mu, sigma, weight, fixerloops] = maximum(pcompx, data, N, D, C)
	% pcompx, data, N, D, C
	...
	% mu, sigma, weight, fixerloops
end


function [pcompx, loglike] = expect(mu, sigma, weight, C, data)
	% mu, sigma, weight, data, C
	% finish test
	tulo = gmmcpdf(data, mu, sigma, weight);	
	pcompx = tulo ./ (sum(tulo,2)*ones(1,C));
...
function [log_covfixer2, log_loglikes, fixing_cycles] = judgeLoglike(conf, loglike, old_loglike, loops, ...
			log_covfixer2, log_loglikes, fixerloops, ...
			weight, mu, sigma, fixing_cycles)
	% conf, loglike, old_loglike, loops, ...
	% log_covfixer2, log_loglikes, fixerloops, ...
	% weight, mu, sigma, fixing_cycles
	...
	% log_covfixer2, log_loglikes, fixing_cycles
end