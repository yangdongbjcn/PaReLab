function b = ImageBlkprocMat(varargin)
...
if (isempty(firstBlock))
  style = 'e'; % empty
  b = [];
elseif ( size(firstBlock,1) == size(x,1) & size(firstBlock,2) == size(x,2) ) % (all(size(firstBlock) == size(x)))   %dyang
  style = 's'; % same
  % Preallocate output.
...
elseif (size(firstBlock,1) == size(x,1) - 2 * border(1) & size(firstBlock,2) == size(x,2) - 2 * border(2))
  style = 'b'; % border
  % Preallocate output.
  if (isa(firstBlock, 'uint8'))
      b = repmat(uint8(0), ma+mpad, na+npad);
