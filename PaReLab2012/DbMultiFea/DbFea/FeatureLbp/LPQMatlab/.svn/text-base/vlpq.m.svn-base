...
%
%
% Version published in 2011 by Juhani Päivärinta, Esa Rahtu, and Janne Heikkil?
...
%% Initialization
vimg = single(vimg);    % convert vimg to double
...
w0 = single((x*0+1));
w1 = single(exp(complex(0,-2*pi*x*STFTalpha)));
w2 = single(conj(w1));
w0d = single(reshape((z*0+1),1,1,planes));
w1d = single(reshape(exp(complex(0,-2*pi*z*STFTalpha2)),1,1,planes));
w2d = single(reshape(conj(w1d),1,1,planes));
...
if L == 10                              % a faster calculation if L=10
    ...
    VLPQdesc = single(zeros(freqRow,1)); % Initialize VLPQ code word
    for i=1:L
        VLPQdesc = VLPQdesc+(single(freqResp(:,i)))*(2^(i-1));
    end
end
